...
contract QuoterV2 is IQuoterV2, IThrusterSwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for IThrusterPool;
    ...
    function thrusterSwapCallback(int256 amount0Delta, int256 amount1Delta, bytes memory path) external view override {
        ...
        IThrusterPool pool = getPool(tokenIn, tokenOut, fee);
    ...
    function handleRevert(bytes memory reason, IThrusterPool pool, uint256 gasEstimate)
        private
        view
        ...
        returns (uint256 amountOut, uint160 sqrtPriceX96After, uint32 initializedTicksCrossed, uint256 gasEstimate)
    {
        ...
        IThrusterPool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
